# ==============================================================
# PacketInfo.py
# Représentation abstraite d’un paquet réseau capturé par Scapy
# Cette classe extrait et stocke les informations utiles
# ==============================================================
...
# ==============================================================
# 2. Mapping des flags TCP (Scapy → lisible)
# ==============================================================
flags = {
    'F': 'FIN',
    'S': 'SYN',
...
# ==============================================================
# 3. Classe PacketInfo
# Chaque instance correspond à un paquet réseau
# ==============================================================
class PacketInfo:
    ...
    # ==========================================================
    # 3.3 Ports + association au processus (psutil)
    # ==========================================================
    def setSrcPort(self, p):
        ...
        # Tentative d’association paquet → processus local
        if self.pid is None and self.p_name == '':
            connections = psutil.net_connections()
            for con in connections:
                if (con.laddr.port - self.src_port == 0.0) or \
                   (con.laddr.port - self.dest_port == 0.0):
                    self.pid = con.pid
    ...
    def setPSHFlag(self, p):
        if p.haslayer(TCP):
            if 'PSH' in [flags[x] for x in p[TCP].flags]:
                self.PSH_flag = True
    ...
    def setFINFlag(self, p):
        if p.haslayer(TCP):
            if 'FIN' in [flags[x] for x in p[TCP].flags]:
                self.FIN_flag = True
    ...
    def setSYNFlag(self, p):
        if p.haslayer(TCP):
            if 'SYN' in [flags[x] for x in p[TCP].flags]:
                self.SYN_flag = True
    ...
    def setACKFlag(self, p):
        if p.haslayer(TCP):
            if 'ACK' in [flags[x] for x in p[TCP].flags]:
                self.ACK_flag = True
    ...
    def setURGFlag(self, p):
        if p.haslayer(TCP):
            if 'URG' in [flags[x] for x in p[TCP].flags]:
                self.URG_flag = True
    ...
    def setRSTFlag(self, p):
        if p.haslayer(TCP):
            if 'RST' in [flags[x] for x in p[TCP].flags]:
                self.RST_flag = True
    ...
    # ==========================================================
    # 3.8 Identifiants de flow (clé bidirectionnelle)
    # ==========================================================
    def setFwdID(self):
        self.fwd_id = (
            self.src + "-" + self.dest + "-" +
